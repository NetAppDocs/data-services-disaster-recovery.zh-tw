...
summary: NetApp災難復原採用角色來管理每個使用者對特定功能和操作的存取權限。 
---
= NetApp災難復原基於角色的功能訪問
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
NetApp災難復原採用角色來管理每個使用者對特定功能和操作的存取權限。
...
這些角色特定於NetApp災難恢復，與NetApp控制台中使用的平台角色不同。有關所有NetApp控制台平台角色的詳細信息，請參閱 https://docs.netapp.com/us-en/bluexp-setup-admin/reference-iam-predefined-roles.html["NetApp控制台設定與管理文檔"^]。

下表列出了每個NetApp災難復原角色可以執行的動作。
