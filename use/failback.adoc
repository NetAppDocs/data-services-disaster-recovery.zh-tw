...
在此工作流程中， NetApp Disaster Recovery 會將所有變更複製（重新同步）回原始來源虛擬機，然後再反轉複製方向。此程序從已完成故障轉移到目標的關係開始，並涉及以下步驟：
...
. 選擇複製計劃。
. 在右側，選擇“操作”選項image:../use/icon-horizontal-dots.png["NetApp災難復原服務​​中的操作選單圖標"]並選擇*故障恢復*。
. 輸入複製計劃名稱確認並開始故障回覆。
